...
// 闪兑前置校验：提现被禁止时抛出业务错误
// GET /api/user/swap/check
export async function checkSwapAllowed() {
  const response = await request("/api/user/swap/check", {
    method: "GET",
  });
  return response;
}
...
// 上期收益说明（问号/帮助弹窗用）
export async function getLastPeriodEarningsHelp() {
  const response = await request("/api/user/last-period/help", {
    method: "GET",
  });
  return response;
}
...
// 		"teamTotalCount": 0,
// 		"teamWhitelistCount": 0,
// 		"teamStakerCount": 0,
// 		"personalStake": 0,
// 		"directValidUserCount": 0,
// 		"teamTodayWhitelistCount": 0,
// 		"bigAreaStake": 0,
// 		"teamTotalStake": 0,
// 		"todayTotalStake": 0,
// 		"smallAreaStakeYesterdayDelta": 0,
// 		"smallAreaStakeChangeRate": 0,
// 		"bigAreaStakeYesterdayDelta": 0,
// 		"bigAreaStakeChangeRate": 0,
// 		"teamTotalStakeYesterdayDelta": 0,
// 		"teamTotalStakeChangeRate": 0,
// 		"teamIncome": 0,
// 		"referralIncome": 0,
...
// 	"success": true
// }
export async function getAssetLedgerPage({
  page = 0,
  limit,
  searchCount,
  lastId,
 ...
 * @param {boolean} [params.searchCount]
...
export async function getArticleList({
  ...
  const headers = {};
  if (language) {
    headers["Accept-Language"] = getBcp47Tag(language);
  }
