...
/** TODO: 接入提现接口后替换为真实余额 */
const MOCK_WITHDRAWABLE_USDT = 888_888;
const MOCK_AVAILABLE_AUL = 888_888;
const MOCK_FEE_USDT = 10;
...
export function WithdrawUSDT({ open, onClose }: WithdrawUSDTProps) {
  ...
  const exceedsLimit =
    parsedAmount != null && parsedAmount > MOCK_WITHDRAWABLE_USDT;
  const belowMin =
  ...
  const handleFillAll = () => {
    setAmount(String(MOCK_WITHDRAWABLE_USDT));
  };
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <InfoRow
                      ...
                      value={`${formatBalance(MOCK_WITHDRAWABLE_USDT)} USDT`}
                    />
                    <InfoRow
                      label={t("mine.availableAul")}
                      value={`${formatBalance(MOCK_AVAILABLE_AUL)} AUL`}
                    />
                    <InfoRow
                      label={t("mine.withdrawFee")}
                      value={`${MOCK_FEE_USDT} USDT + ${MOCK_FEE_AUL} AUL`}
                    />
                  </div>
                </div>
              </FieldBlock>
